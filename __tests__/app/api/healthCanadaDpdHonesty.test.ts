/**
 * healthCanadaProducts panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/healthCanadaDpd', () => ({
  getHealthCanadaProductsByName: jest.fn(),
}))
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:healthCanadaProducts',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/healthCanadaProducts',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('healthCanadaProducts panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getHealthCanadaProductsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'healthCanadaProducts' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getHealthCanadaProductsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'healthCanadaProducts' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getHealthCanadaProductsByName as jest.Mock).mockResolvedValue([{ drugCode: 1, din: '123', brandName: 'Aspirin', companyName: 'Bayer', className: 'Human', descriptor: '', numberOfAis: '1', lastUpdateDate: '', status: 'Marketed', historyDate: '', originalMarketDate: '', forms: [], routes: [], ingredients: [], url: 'https://health-products.canada.ca/' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'healthCanadaProducts' } })
    const json = await res.json()
