/**
 * nciThesaurus panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/nciThesaurus',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('nciThesaurus panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getNciConceptsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'nciThesaurus' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getNciConceptsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'nciThesaurus' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getNciConceptsByName as jest.Mock).mockResolvedValue([{ code: 'C61948', name: 'Aspirin' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'nciThesaurus' } })
    const json = await res.json()
