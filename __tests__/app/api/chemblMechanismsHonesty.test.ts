/**
 * chemblMechanisms panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/chembl-mechanisms', () => ({
  getChemblMechanismsByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/chemblMechanisms',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('chemblMechanisms panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getChemblMechanismsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'chemblMechanisms' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getChemblMechanismsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'chemblMechanisms' } })
    const json = await res.json()
