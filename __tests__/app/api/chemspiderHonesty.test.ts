/**
 * chemSpiderCompounds panel leaf: HTTP errors must not look like honest EMPTY.
 */
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:chemSpiderCompounds',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/chemSpiderCompounds',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('chemSpiderCompounds panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchChemSpider as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'chemSpiderCompounds' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchChemSpider as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'chemSpiderCompounds' } })
    const json = await res.json()
