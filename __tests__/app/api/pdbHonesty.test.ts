/**
 * pdbStructures panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/pdb', () => ({
  getPdbStructuresByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/pdbStructures',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('pdbStructures panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getPdbStructuresByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pdbStructures' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getPdbStructuresByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pdbStructures' } })
    const json = await res.json()
