/**
 * humanProteinAtlas panel leaf: HTTP errors must not look like honest EMPTY.
 */
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:humanProteinAtlas',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/humanProteinAtlas',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('humanProteinAtlas panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getProteinAtlasData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'humanProteinAtlas' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getProteinAtlasData as jest.Mock).mockResolvedValue(null)
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'humanProteinAtlas' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getProteinAtlasData as jest.Mock).mockResolvedValue({
      gene: 'TP53',
      ensemblId: 'ENSG00000141510',
      tissueExpression: [{ tissue: 'liver', tissueType: 'organ', expressionLevel: 'High', score: 3, nRna: 10, nProtein: 8 }],
    })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'humanProteinAtlas' } })
    const json = await res.json()
    expect(json._agentStatus).toBe('loaded')
    expect(json.data).toHaveLength(1)
    expect(json.data[0].gene).toBe('TP53')
  })
})
