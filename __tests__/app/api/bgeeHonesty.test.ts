/**
 * bgeeExpression panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/bgeeExpression',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('bgeeExpression panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getBgeeData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'bgeeExpression' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getBgeeData as jest.Mock).mockResolvedValue({ expressions: [] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'bgeeExpression' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getBgeeData as jest.Mock).mockResolvedValue({
      expressions: [
        {
          geneId: 'BRCA1',
          geneSymbol: 'BRCA1',
          species: 'Homo sapiens',
          anatomicalEntityId: 'UBERON:0000955',
          anatomicalEntityName: 'brain',
