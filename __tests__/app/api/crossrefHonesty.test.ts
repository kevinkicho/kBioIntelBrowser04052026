/**
 * Crossref panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/crossref', () => ({
  searchCrossRef: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/crossRefWorks',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('crossref panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchCrossRef as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'crossRefWorks' } })
  ...
  it('true empty works is empty agent status, not error', async () => {
    ;(searchCrossRef as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'crossRefWorks' } })
    const json = await res.json()
