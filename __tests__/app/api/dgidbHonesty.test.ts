/**
 * drugGeneInteractions panel leaf: HTTP errors must not look like honest EMPTY.
 */
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:drugGeneInteractions',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/drugGeneInteractions',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('drugGeneInteractions panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ...
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugGeneInteractions' } })
  ...
  it('rows are loaded', async () => {
    ;(getDrugGeneInteractionsByName as jest.Mock).mockResolvedValue([
      { geneName: 'PTGS2', interactionType: 'inhibitor' },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugGeneInteractions' } })
    const json = await res.json()
