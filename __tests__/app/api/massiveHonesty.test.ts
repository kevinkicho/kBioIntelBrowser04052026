/**
 * massiveDatasets panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/massive', () => ({
  searchMassive: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/massiveDatasets',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('massiveDatasets panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchMassive as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'massiveDatasets' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchMassive as jest.Mock).mockResolvedValue({ datasets: [], total: 0 })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'massiveDatasets' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(searchMassive as jest.Mock).mockResolvedValue({ datasets: [{ id: 'MSV000001', title: 'Aspirin proteomics', description: '', doi: '', submitter: '', submissionDate: '', updateDate: '', organism: '', instrumentType: '', datasetType: '', sampleType: '', lab: '', contactName: '', contactEmail: '', fileCount: 0, fileSize: 0, url: 'https://massive.ucsd.edu/' }], total: 1 })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'massiveDatasets' } })
    const json = await res.json()
