/**
 * medGenConcepts panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/medgen', () => ({
  getMedGenConcepts: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/medGenConcepts',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('medGenConcepts panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getMedGenConcepts as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'medGenConcepts' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getMedGenConcepts as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'medGenConcepts' } })
    const json = await res.json()
    ...
    ;(getMedGenConcepts as jest.Mock).mockResolvedValue([
      {
        ...
        umlsCui: 'C0004096',
        omimIds: [],
        url: 'https://www.ncbi.nlm.nih.gov/medgen/10911',
      },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'medGenConcepts' } })
    const json = await res.json()
