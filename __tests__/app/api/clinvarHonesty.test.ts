/**
 * clinVarVariants panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/clinvar', () => ({
  getClinVarVariantsByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/clinVarVariants',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('clinVarVariants panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getClinVarVariantsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'clinVarVariants' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getClinVarVariantsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'clinVarVariants' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getClinVarVariantsByName as jest.Mock).mockResolvedValue([
      {
        variantId: '12345',
        title: 'NM_000044.6(AR):c.2596G>A',
