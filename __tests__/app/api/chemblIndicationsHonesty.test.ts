/**
 * chemblIndications panel leaf: HTTP errors must not look like honest EMPTY.
 */
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:chemblIndications',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/chemblIndications',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('chemblIndications panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ...
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'chemblIndications' } })
  ...
  it('rows are loaded', async () => {
    ;(getChemblIndicationsByName as jest.Mock).mockResolvedValue([
      { meshHeading: 'Pain', maxPhaseForIndication: 4 },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'chemblIndications' } })
    const json = await res.json()
