/**
 * ctdData panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/ctdData',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('ctdData panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getCTDData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'ctdData' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getCTDData as jest.Mock).mockResolvedValue({ interactions: [], diseaseAssociations: [] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'ctdData' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getCTDData as jest.Mock).mockResolvedValue({
      interactions: [{
