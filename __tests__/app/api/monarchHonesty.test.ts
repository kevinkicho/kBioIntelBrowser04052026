/**
 * Monarch panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/monarch', () => ({
  getMonarchDiseasesByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/monarchDiseases',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('monarchDiseases panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getMonarchDiseasesByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'monarchDiseases' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getMonarchDiseasesByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'monarchDiseases' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getMonarchDiseasesByName as jest.Mock).mockResolvedValue([{ id: 'MONDO:0011993', name: 'type 2 diabetes mellitus' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'monarchDiseases' } })
    const json = await res.json()
