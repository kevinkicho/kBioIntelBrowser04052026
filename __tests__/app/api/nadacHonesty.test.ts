/**
 * drugPrices panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/drugPrices',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('drugPrices panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getDrugPricesByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugPrices' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getDrugPricesByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugPrices' } })
    const json = await res.json()
