/**
 * peptideAtlas panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/peptideAtlas',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('peptideAtlas panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getPeptideAtlasData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'peptideAtlas' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getPeptideAtlasData as jest.Mock).mockResolvedValue({ peptides: [] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'peptideAtlas' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getPeptideAtlasData as jest.Mock).mockResolvedValue({
      peptides: [{ peptideId: 'P1', sequence: 'PEPTIDE' }],
    })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'peptideAtlas' } })
    const json = await res.json()
