/**
 * meshTerms panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/meshTerms',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('meshTerms panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getMeshTermsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'meshTerms' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getMeshTermsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'meshTerms' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getMeshTermsByName as jest.Mock).mockResolvedValue([{ meshId: 'D001241', name: 'Aspirin' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'meshTerms' } })
    const json = await res.json()
