/**
 * Open Targets panel leaf: HTTP errors must not look like honest EMPTY.
 */
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:diseaseAssociations',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/diseaseAssociations',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('diseaseAssociations panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ...
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'diseaseAssociations' } })
  ...
  it('rows are loaded', async () => {
    ;(getDiseaseAssociationsByName as jest.Mock).mockResolvedValue([{ diseaseId: 'EFO_0000311', diseaseName: 'Type 2 diabetes mellitus' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'diseaseAssociations' } })
    const json = await res.json()
