/**
 * ChEMBL panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/chemblActivities',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('chembl panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ...
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'chemblActivities' } })
  ...
  it('rows are loaded', async () => {
    ;(getChemblActivitiesByName as jest.Mock).mockResolvedValue([{ targetName: 'COX-2', chemblId: 'CHEMBL25' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'chemblActivities' } })
    const json = await res.json()
