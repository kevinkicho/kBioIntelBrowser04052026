/**
 * NIH RePORTER panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/nihGrants',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('nihGrants panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ...
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'nihGrants' } })
  ...
  it('rows are loaded', async () => {
    ;(getNihGrantsByName as jest.Mock).mockResolvedValue([{ projectNumber: 'R01X', title: 'Grant' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'nihGrants' } })
    const json = await res.json()
