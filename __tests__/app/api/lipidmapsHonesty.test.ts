/**
 * lipidMapsLipids panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/lipidMapsLipids',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('lipidMapsLipids panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchLipidMaps as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'lipidMapsLipids' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchLipidMaps as jest.Mock).mockResolvedValue({ lipids: [], total: 0 })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'lipidMapsLipids' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(searchLipidMaps as jest.Mock).mockResolvedValue({
      lipids: [{ lmId: 'LMFA01010001', name: 'hexadecanoic acid', synonyms: [], category: '', mainClass: '', subClass: '', formula: 'C16H32O2', molecularWeight: 256, exactMass: 256, url: 'https://www.lipidmaps.org/data/structure/LMFA01010001' }],
      total: 1,
    })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'lipidMapsLipids' } })
    const json = await res.json()
