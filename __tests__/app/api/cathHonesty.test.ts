/**
 * cathDomains panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/cathDomains',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('cathDomains panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchCATHDomains as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'cathDomains' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchCATHDomains as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'cathDomains' } })
    const json = await res.json()
    ...
    ;(searchCATHDomains as jest.Mock).mockResolvedValue([
      {
        ...
        sequence: '',
        length: 129,
        url: 'http://www.cathdb.info/domain/1cukA00',
      },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'cathDomains' } })
    const json = await res.json()
