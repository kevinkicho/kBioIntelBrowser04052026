/**
 * biologicsLicensed panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/biologicsLicensed', () => ({
  getBiologicsLicensedByName: jest.fn(),
}))
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:biologicsLicensed',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/biologicsLicensed',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('biologicsLicensed panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getBiologicsLicensedByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'biologicsLicensed' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getBiologicsLicensedByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'biologicsLicensed' } })
    const json = await res.json()
    ...
    ;(getBiologicsLicensedByName as jest.Mock).mockResolvedValue([
      {
        ...
        establishmentSearchUrl: 'https://www.fda.gov/search?s=ABBVIE',
      },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'biologicsLicensed' } })
    const json = await res.json()
