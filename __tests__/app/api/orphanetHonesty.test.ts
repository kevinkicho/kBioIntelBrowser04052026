/**
 * orphanetData panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/orphanetData',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('orphanetData panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getOrphanetData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'orphanetData' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getOrphanetData as jest.Mock).mockResolvedValue({ diseases: [] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'orphanetData' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getOrphanetData as jest.Mock).mockResolvedValue({
      diseases: [{ orphaCode: '18', diseaseName: 'ATTR amyloidosis' }],
    })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'orphanetData' } })
    const json = await res.json()
