/**
 * nsfAwards panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/nsfAwards', () => ({
  getNsfAwardsByKeyword: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/nsfAwards',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('nsfAwards panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getNsfAwardsByKeyword as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'nsfAwards' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getNsfAwardsByKeyword as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'nsfAwards' } })
    const json = await res.json()
    ...
    ;(getNsfAwardsByKeyword as jest.Mock).mockResolvedValue([
      {
        ...
        endDate: '12/31/2023',
        abstract: 'Study of protein folding markers.',
        awardUrl: 'https://www.nsf.gov/awardsearch/showAward?AWD_ID=1234567',
      },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'nsfAwards' } })
    const json = await res.json()
