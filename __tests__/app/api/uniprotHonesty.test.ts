/**
 * UniProt panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/uniprot', () => ({
  getUniprotEntriesByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/uniprotEntries',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('uniprot panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getUniprotEntriesByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'uniprotEntries' } })
  ...
  it('rows are loaded', async () => {
    ;(getUniprotEntriesByName as jest.Mock).mockResolvedValue([{ accession: 'P00734', proteinName: 'Prothrombin' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'uniprotEntries' } })
    const json = await res.json()
