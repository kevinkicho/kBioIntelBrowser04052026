/**
 * Reactome panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/reactome', () => ({
  getReactomePathwaysByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/reactomePathways',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('reactome panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getReactomePathwaysByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'reactomePathways' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getReactomePathwaysByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'reactomePathways' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getReactomePathwaysByName as jest.Mock).mockResolvedValue([{ stId: 'R-HSA-1', name: 'Pathway' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'reactomePathways' } })
    const json = await res.json()
