/**
 * pharosTargets panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/pharos', () => ({
  getPharosTargetsByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/pharosTargets',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('pharosTargets panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getPharosTargetsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pharosTargets' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getPharosTargetsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pharosTargets' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getPharosTargetsByName as jest.Mock).mockResolvedValue([
      { name: 'PTGS2', tdl: 'Tclin', family: 'Enzyme' },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pharosTargets' } })
    const json = await res.json()
