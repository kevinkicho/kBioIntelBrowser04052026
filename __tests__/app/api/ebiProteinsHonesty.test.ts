/**
 * proteinFeatures panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/ebi-proteins', () => ({
  getProteinFeaturesByAccessions: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/proteinFeatures',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('proteinFeatures panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getProteinFeaturesByAccessions as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'proteinFeatures' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getProteinFeaturesByAccessions as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'proteinFeatures' } })
    const json = await res.json()
