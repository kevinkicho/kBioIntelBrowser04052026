/**
 * metabolightsStudies panel leaf: HTTP errors must not look like honest EMPTY.
 */
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:metabolightsStudies',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/metabolightsStudies',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('metabolightsStudies panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchMetaboLights as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'metabolightsStudies' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchMetaboLights as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'metabolightsStudies' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(searchMetaboLights as jest.Mock).mockResolvedValue([{ id: 'MTBLS1', title: 'Aspirin metabolomics', description: '', studyType: 'Metabolomics', organism: '', organismPart: '', platform: '', metabolites: 0, samples: 0, techniques: [], publication: '', publicationDate: '', url: 'https://www.ebi.ac.uk/metabolights/MTBLS1' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'metabolightsStudies' } })
    const json = await res.json()
