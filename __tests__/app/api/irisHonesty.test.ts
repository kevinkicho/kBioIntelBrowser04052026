/**
 * irisAssessments panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/irisAssessments',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('irisAssessments panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchIRIS as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'irisAssessments' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchIRIS as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'irisAssessments' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(searchIRIS as jest.Mock).mockResolvedValue([{ id: 'DTXSID3039242', chemicalName: 'Benzene', casNumber: '71-43-2', hasIrisData: true }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'irisAssessments' } })
    const json = await res.json()
