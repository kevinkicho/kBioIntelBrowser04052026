/**
 * drugInteractions panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/rxnorm', () => ({
  getDrugInteractionsByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/drugInteractions',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('drugInteractions panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getDrugInteractionsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugInteractions' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getDrugInteractionsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugInteractions' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getDrugInteractionsByName as jest.Mock).mockResolvedValue([{ drugName: 'warfarin', severity: 'moderate', description: 'x', sourceName: 'DrugBank' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugInteractions' } })
    const json = await res.json()
