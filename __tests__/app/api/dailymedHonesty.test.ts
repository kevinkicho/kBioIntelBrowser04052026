/**
 * drugLabels panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/drugLabels',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('drugLabels panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getDrugLabelsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugLabels' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getDrugLabelsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugLabels' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getDrugLabelsByName as jest.Mock).mockResolvedValue([
      { title: 'ASPIRIN tablet', setId: 'abc' },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugLabels' } })
    const json = await res.json()
