/**
 * disgenetData panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/disgenetData',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('disgenetData panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getDisGeNetData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'disgenetData' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getDisGeNetData as jest.Mock).mockResolvedValue({ associations: [] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'disgenetData' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getDisGeNetData as jest.Mock).mockResolvedValue({
      associations: [{ geneSymbol: 'TTR', diseaseName: 'ATTR' }],
    })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'disgenetData' } })
    const json = await res.json()
