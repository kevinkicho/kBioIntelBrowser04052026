/**
 * expressionAtlas panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/expression-atlas', () => ({
  getGeneExpressionBySymbols: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/expressionAtlas',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('expressionAtlas panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    ;(getMoleculeById as jest.Mock).mockResolvedValue({ name: 'ACE', cid: 2244, synonyms: [] })
  })

  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getGeneExpressionBySymbols as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'expressionAtlas' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getGeneExpressionBySymbols as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'expressionAtlas' } })
    const json = await res.json()
    ...
    ;(getGeneExpressionBySymbols as jest.Mock).mockResolvedValue([
      {
        ...
        experimentDescription: 'Human baseline',
        species: 'Homo sapiens',
        url: 'https://www.ebi.ac.uk/gxa/experiments/E-MTAB-1',
      },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'expressionAtlas' } })
    const json = await res.json()
