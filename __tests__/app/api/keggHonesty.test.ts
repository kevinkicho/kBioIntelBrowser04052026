/**
 * keggData panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/keggData',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('keggData panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getKEGGData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'keggData' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getKEGGData as jest.Mock).mockResolvedValue({ pathways: [], compounds: [], drugs: [] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'keggData' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getKEGGData as jest.Mock).mockResolvedValue({
