/**
 * MyGene panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/myGeneData',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('myGeneData panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getMyGeneData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'myGeneData' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getMyGeneData as jest.Mock).mockResolvedValue({ genes: [] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'myGeneData' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getMyGeneData as jest.Mock).mockResolvedValue({ genes: [{ symbol: 'ACE', geneId: '1636' }] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'myGeneData' } })
    const json = await res.json()
