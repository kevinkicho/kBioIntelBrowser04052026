/**
 * PubMed panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/pubmedArticles',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('pubmed panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchPubMed as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pubmedArticles' } })
  ...
  it('true empty articles is empty agent status, not error', async () => {
  ...
  it('rows are loaded', async () => {
    ;(searchPubMed as jest.Mock).mockResolvedValue([{ pmid: '1', title: 'Paper' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pubmedArticles' } })
    const json = await res.json()
