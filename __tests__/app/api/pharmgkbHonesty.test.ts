/**
 * pharmgkb panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/pharmgkb',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('pharmgkb panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getPharmGKBData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pharmgkb' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getPharmGKBData as jest.Mock).mockResolvedValue({ drugs: [], genes: [], guidelines: [] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pharmgkb' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getPharmGKBData as jest.Mock).mockResolvedValue({
      drugs: [{ id: 'PA450428', name: 'warfarin', genericNames: [], brandNames: [], drugClass: '', fdaApproval: '', genes: [], guidelines: [], url: 'https://www.pharmgkb.org/drug/PA450428' }],
      genes: [],
      guidelines: [],
    })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pharmgkb' } })
