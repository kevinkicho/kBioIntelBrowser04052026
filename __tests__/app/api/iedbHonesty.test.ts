/**
 * iedbData panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/iedbData',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('iedbData panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getIEDBData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'iedbData' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getIEDBData as jest.Mock).mockResolvedValue({ epitopes: [] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'iedbData' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getIEDBData as jest.Mock).mockResolvedValue({ epitopes: [{ epitopeId: 1, sequence: "PEPTIDE" }] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'iedbData' } })
    const json = await res.json()
