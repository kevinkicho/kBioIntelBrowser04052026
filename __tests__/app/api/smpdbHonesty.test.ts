/**
 * smpdbPathways panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/smpdbPathways',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('smpdbPathways panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchSMPDB as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'smpdbPathways' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchSMPDB as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'smpdbPathways' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(searchSMPDB as jest.Mock).mockResolvedValue([
      {
        smpdbId: 'R-HSA-2162123',
        name: 'Synthesis of Prostaglandins (PG)',
        description: 'Prostaglandins are synthesized from arachidonic acid.',
