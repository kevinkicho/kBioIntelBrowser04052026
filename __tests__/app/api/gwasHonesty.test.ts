/**
 * GWAS Catalog panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/gwas-catalog', () => ({
  getGwasAssociationsByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/gwasAssociations',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('gwasAssociations panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getGwasAssociationsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'gwasAssociations' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getGwasAssociationsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'gwasAssociations' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getGwasAssociationsByName as jest.Mock).mockResolvedValue([{ traitName: 'Type 2 Diabetes', pValue: 1e-8 }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'gwasAssociations' } })
    const json = await res.json()
