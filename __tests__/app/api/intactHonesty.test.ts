/**
 * molecularInteractions panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/intact', () => ({
  getMolecularInteractionsByName: jest.fn(),
}))
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:molecularInteractions',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/molecularInteractions',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('molecularInteractions panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getMolecularInteractionsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'molecularInteractions' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getMolecularInteractionsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'molecularInteractions' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getMolecularInteractionsByName as jest.Mock).mockResolvedValue([{ interactorA: 'EGFR', interactorB: 'ERBB2', interactionId: 'EBI-1' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'molecularInteractions' } })
    const json = await res.json()
