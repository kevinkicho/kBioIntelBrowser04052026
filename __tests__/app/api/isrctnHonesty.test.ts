/**
 * isrctnTrials panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/isrctn', () => ({
  searchISRCTN: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/isrctnTrials',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('isrctnTrials panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchISRCTN as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'isrctnTrials' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchISRCTN as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'isrctnTrials' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(searchISRCTN as jest.Mock).mockResolvedValue([{ isRCTN: "ISRCTN1", title: "t", status: "Recruiting", phase: "", recruitmentStatus: "", sponsor: "", country: "", startDate: "", endDate: "", targetEnrollment: 0, conditions: [], interventions: [], outcomes: [], url: "https://example.com" }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'isrctnTrials' } })
    const json = await res.json()
