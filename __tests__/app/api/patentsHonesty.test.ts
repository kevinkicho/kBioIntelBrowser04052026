/**
 * Patents leaf: PubChem HTTP 5xx must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/patents', () => ({
  getPatentsByMoleculeName: jest.fn(),
}))
...
import { GET as patentsGET } from '@/app/api/patents/[id]/route'
...
describe('patents leaf honesty', () => {
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getPatentsByMoleculeName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await patentsGET(fakeReq(), { params: { id: '2244' } })
  ...
  it('true empty xrefs is empty agent status, not error', async () => {
