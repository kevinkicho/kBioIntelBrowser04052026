/**
 * usHospitals panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/cmsHospitals', () => ({
  searchCmsHospitalsByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/usHospitals',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('usHospitals panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchCmsHospitalsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'usHospitals' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchCmsHospitalsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'usHospitals' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(searchCmsHospitalsByName as jest.Mock).mockResolvedValue([{
      facilityId: '240010',
      facilityName: 'MAYO CLINIC HOSPITAL ROCHESTER',
