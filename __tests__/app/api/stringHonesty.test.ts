/**
 * STRING panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/string-db', () => ({
  getProteinInteractionsByName: jest.fn(),
}))
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:proteinInteractions',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/proteinInteractions',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('proteinInteractions panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getProteinInteractionsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'proteinInteractions' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getProteinInteractionsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'proteinInteractions' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getProteinInteractionsByName as jest.Mock).mockResolvedValue([{ proteinA: 'ACE', proteinB: 'AGT', score: 0.9 }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'proteinInteractions' } })
    const json = await res.json()
