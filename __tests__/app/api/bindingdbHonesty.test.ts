/**
 * BindingDB panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/bindingdb', () => ({
  getBindingAffinitiesByName: jest.fn(),
}))
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:bindingAffinities',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/bindingAffinities',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('bindingAffinities panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ...
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'bindingAffinities' } })
  ...
  it('rows are loaded', async () => {
    ;(getBindingAffinitiesByName as jest.Mock).mockResolvedValue([{ targetName: 'GLP1R', affinityType: 'Ki', affinityValue: 1 }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'bindingAffinities' } })
    const json = await res.json()
