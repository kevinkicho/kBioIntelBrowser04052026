/**
 * drugCentral panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/drugcentral', () => ({
  getDrugCentralData: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/drugCentral',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('drugCentral panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getDrugCentralData as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugCentral' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getDrugCentralData as jest.Mock).mockResolvedValue({ drug: null, targets: [] })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugCentral' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getDrugCentralData as jest.Mock).mockResolvedValue({
      drug: { id: 1, name: 'aspirin' },
      targets: [{ targetName: 'PTGS1' }],
    })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'drugCentral' } })
    const json = await res.json()
