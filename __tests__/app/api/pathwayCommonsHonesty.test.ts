/**
 * pathwayCommons panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/pathway-commons', () => ({
  getPathwayCommonsByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/pathwayCommons',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('pathwayCommons panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getPathwayCommonsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pathwayCommons' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getPathwayCommonsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pathwayCommons' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getPathwayCommonsByName as jest.Mock).mockResolvedValue([
      { pathwayId: 'R-HSA-123', pathwayName: 'Aspirin Metabolism', name: 'Aspirin Metabolism' },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pathwayCommons' } })
    const json = await res.json()
