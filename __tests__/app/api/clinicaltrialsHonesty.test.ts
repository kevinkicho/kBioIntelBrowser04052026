/**
 * ClinicalTrials leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/clinicaltrials', () => ({
  getClinicalTrialsByName: jest.fn(),
}))
...
import { GET as trialsGET } from '@/app/api/clinicaltrials/[id]/route'
...
describe('clinicaltrials leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getClinicalTrialsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await trialsGET(fakeReq(), { params: { id: '2244' } })
    const json = await res.json()
    expect(json._agentStatus).toBe('error')
    expect(json._partial).toBe(true)
    expect(json._emptyHonest).toBeUndefined()
    expect(json.trials).toEqual([])
    expect(classifyHonestyEnvelope(json)).toBe('ERROR')
    expect(shouldCacheHonestyEnvelope(json)).toBe(false)
  })

  it('true empty studies is empty agent status, not error', async () => {
    ;(getClinicalTrialsByName as jest.Mock).mockResolvedValue([])
    const res = await trialsGET(fakeReq(), { params: { id: '2244' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getClinicalTrialsByName as jest.Mock).mockResolvedValue([{ nctId: 'NCT1' }])
    const res = await trialsGET(fakeReq(), { params: { id: '2244' } })
    const json = await res.json()
    expect(json._agentStatus).toBe('loaded')
    expect(json.trials).toHaveLength(1)
  })
})
