/**
 * sabdabEntries panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/sabdabEntries',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('sabdabEntries panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchSAbDab as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'sabdabEntries' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchSAbDab as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'sabdabEntries' } })
    const json = await res.json()
    ...
    ;(searchSAbDab as jest.Mock).mockResolvedValue([
      {
        ...
        antibodyType: 'Fab',
        cdrSequences: {
          heavy: { cdr1: '', cdr2: '', cdr3: '' },
          light: { cdr1: '', cdr2: '', cdr3: '' },
        },
        affinity: null,
        affinityUnits: 'nM',
        url: 'https://opig.stats.ox.ac.uk/webapps/abdb/1a2y',
      },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'sabdabEntries' } })
    const json = await res.json()
