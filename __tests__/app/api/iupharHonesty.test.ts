/**
 * pharmacologyTargets panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/iuphar', () => ({
  getPharmacologyTargetsByName: jest.fn(),
}))
  ...
  freeApiAgent: async (spec: {
    ...
    try {
      const data = await spec.run({
        ...
        attempt: 1,
        source: 'panel:pharmacologyTargets',
      })
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/pharmacologyTargets',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('pharmacologyTargets panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getPharmacologyTargetsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pharmacologyTargets' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getPharmacologyTargetsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pharmacologyTargets' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getPharmacologyTargetsByName as jest.Mock).mockResolvedValue([
      { targetName: 'PTGS2', actionType: 'inhibitor' },
    ])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'pharmacologyTargets' } })
    const json = await res.json()
