/**
 * openAireProjects panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/openaire', () => ({
  getEuResearchProjectsByName: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/openAireProjects',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('openAireProjects panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getEuResearchProjectsByName as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'openAireProjects' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getEuResearchProjectsByName as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'openAireProjects' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getEuResearchProjectsByName as jest.Mock).mockResolvedValue([{ id: 'proj-1', code: '101000000', title: 'Aspirin mechanisms', acronym: 'ASPMECH', startDate: '2020-01-01', endDate: '2024-12-31', funderShort: 'EC', funderName: 'European Commission', jurisdiction: 'EU', fundedAmount: 1000000, totalCost: 1200000, url: 'https://explore.openaire.eu/search/project?projectId=proj-1', cordisUrl: 'https://cordis.europa.eu/project/id/101000000' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'openAireProjects' } })
    const json = await res.json()
