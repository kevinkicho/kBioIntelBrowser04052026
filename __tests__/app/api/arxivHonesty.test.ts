/**
 * arXiv panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/arxivPapers',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('arxiv panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchArXiv as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'arxivPapers' } })
  ...
  it('true empty papers is empty agent status, not error', async () => {
    ;(searchArXiv as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'arxivPapers' } })
    const json = await res.json()
