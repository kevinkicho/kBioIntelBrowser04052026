/**
 * companies panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/companies',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('companies panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getDrugsByIngredient as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'companies' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(getDrugsByIngredient as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'companies' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getDrugsByIngredient as jest.Mock).mockResolvedValue([{ company: 'Novo Nordisk', brandName: 'Victoza', genericName: 'LIRAGLUTIDE', productType: 'HUMAN PRESCRIPTION DRUG', route: 'SUBCUTANEOUS' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'companies' } })
    const json = await res.json()
