/**
 * Ensembl panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/ensembl', () => ({
  getEnsemblGenesBySymbols: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/ensemblGenes',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('ensemblGenes panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(getEnsemblGenesBySymbols as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'ensemblGenes' } })
  ...
  it('true empty rows is empty agent status, not error', async () => {
    ;(getEnsemblGenesBySymbols as jest.Mock).mockResolvedValue([])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'ensemblGenes' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(getEnsemblGenesBySymbols as jest.Mock).mockResolvedValue([{ geneId: 'ENSG1', displayName: 'ACE' }])
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'ensemblGenes' } })
    const json = await res.json()
