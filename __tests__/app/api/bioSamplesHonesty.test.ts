/**
 * bioSamples panel leaf: HTTP errors must not look like honest EMPTY.
 */
...
jest.mock('@/lib/api/biosamples', () => ({
  searchBioSamples: jest.fn(),
}))
...
function fakeReq() {
  return {
    url: 'http://localhost/api/molecule/2244/panel/bioSamples',
    signal: undefined,
  } as unknown as import('next/server').NextRequest
}

describe('bioSamples panel leaf honesty', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('HTTP error is ERROR, not EMPTY', async () => {
    ;(searchBioSamples as jest.Mock).mockRejectedValue(new Error('HTTP 503'))
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'bioSamples' } })
  ...
  it('true empty shell is empty agent status, not error', async () => {
    ;(searchBioSamples as jest.Mock).mockResolvedValue({ samples: [], total: 0, page: 0, size: 10 })
    const res = await panelGET(fakeReq(), { params: { id: '2244', panelId: 'bioSamples' } })
    const json = await res.json()
  ...
  it('rows are loaded', async () => {
    ;(searchBioSamples as jest.Mock).mockResolvedValue({
      samples: [{
        id: 'SAMEA1',
        name: 'blood',
        domain: 'self',
