...
export function MassivePanel({
  ...
  const sortOptions = useMemo(
    () => [
      ...dateSortOptions<MassIVEDataset>((d) => d.updateDate || d.submissionDate, {
        newest: 'Newest update',
        oldest: 'Oldest update',
      }),
      ...numberSortOptions<MassIVEDataset>((d) => d.fileCount ?? 0, {
        high: 'Most files',
        low: 'Fewest files',
      }),
      ...alphaSortOptions<MassIVEDataset>((d) => d.title || ''),
    ],
    [],
  )
  ...
  return (
    <Panel panelId={panelId} title={`MassIVE Proteomics (${list.length})`} lastFetched={lastFetched}>
