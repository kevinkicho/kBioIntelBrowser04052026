...
export const MetabolomicsPanel = memo(function MetabolomicsPanel({
  data,
  panelId,
  lastFetched,
}: MetabolomicsPanelProps) {
  ...
  const metaboliteSort = useMemo(
    () => [
      ...alphaSortOptions<MetaboliteData>((m) => m.refmetName || ''),
      ...numberSortOptions<MetaboliteData>((m) => m.exactMass ?? 0, {
        high: 'Highest mass',
        low: 'Lowest mass',
      }),
    ],
  ...
  const studySort = useMemo(
    () => [
      ...alphaSortOptions<MetabolomicsStudy>((s) => s.title || ''),
      ...numberSortOptions<MetabolomicsStudy>((s) => s.metabolites ?? 0, {
        high: 'Most metabolites',
        low: 'Fewest metabolites',
      }),
    ],
    [],
  )
          ...
          {metabolites.length > 0 && (
            <div className="space-y-2">
              ...
              <FilterablePaginatedList
                ...
                getKey={(met, i) => `${met.refmetName || met.hmdbId}-${i}`}
                renderItem={(met) => (
          ...
          {studies.length > 0 && (
            <div className="space-y-2">
              ...
              <FilterablePaginatedList
                items={studies}
                getSearchText={(s) =>
                  [s.title, s.studyId, s.description, s.doi, ...(s.organisms || [])]
                    .filter(Boolean)
                    .join(' ')
                }
                ...
                getKey={(study, i) => `${study.studyId || study.title}-${i}`}
                renderItem={(study) => (
                  <div className="p-3 rounded-lg bg-slate-800/30 border border-slate-700">
                    <a
