...
export const DrugInteractionsPanel = memo(function DrugInteractionsPanel({ interactions, panelId, lastFetched }: { interactions: DrugInteraction[], panelId?: string, lastFetched?: Date }) {
  if (interactions.length === 0) {
    return (
      <Panel title="Drug Interactions (RxNorm)" panelId={panelId} lastFetched={lastFetched}>
        <p className="text-slate-500 text-sm">No drug interactions found for this molecule.</p>
      </Panel>
    )
  }
