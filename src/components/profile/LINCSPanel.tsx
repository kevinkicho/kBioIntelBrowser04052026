...
export function LINCSPanel({
  ...
  const sortOptions = useMemo(
    () => [
      ...numberSortOptions<LINCSSignature>((s) => Math.abs(s.zScore ?? 0), {
        high: 'Highest |z-score|',
        low: 'Lowest |z-score|',
      }),
      ...numberSortOptions<LINCSSignature>((s) => s.pValue ?? 1, {
        high: 'Highest p-value',
        low: 'Lowest p-value',
      }).map((o) => ({ ...o, id: `pval-${o.id}` })),
      ...alphaSortOptions<LINCSSignature>((s) => s.perturbationName || ''),
    ],
    [],
  )
  ...
  return (
    <Panel panelId={panelId} title={`LINCS L1000 (${list.length})`} lastFetched={lastFetched}>
        ...
        <FilterablePaginatedList
          ...
          getSearchText={(s) =>
            [
              ...
              s.tissue,
              s.timePoint,
              ...(s.upregulatedGenes || []).slice(0, 20),
              ...(s.downregulatedGenes || []).slice(0, 20),
            ]
              .filter(Boolean)
              .join(' ')
          }
          ...
          filterPlaceholder="Filter signatures…"
          pageSize={5}
          getKey={(signature) =>
            `${signature.perturbationId}-${signature.cellLine}-${signature.timePoint}`
          }
