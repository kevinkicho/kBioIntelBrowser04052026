...
export const CompToxPanel = memo(function CompToxPanel({ data, panelId, lastFetched }: { data: CompToxData | null, panelId?: string, lastFetched?: Date }) {
  if (!data) {
    return (
      <Panel title="EPA CompTox Dashboard" panelId={panelId} lastFetched={lastFetched}>
        <p className="text-slate-500 text-sm">No CompTox data available for this molecule.</p>
      </Panel>
    )
  }
