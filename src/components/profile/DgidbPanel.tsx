...
export const DgidbPanel = memo(function DgidbPanel({ interactions, panelId, lastFetched }: { interactions: DrugGeneInteraction[], panelId?: string, lastFetched?: Date }) {
  if (interactions.length === 0) {
    return (
      <Panel title="Drug-Gene Interactions (DGIdb)" panelId={panelId} lastFetched={lastFetched}>
        <p className="text-slate-500 text-sm">No drug-gene interactions found for this molecule.</p>
      </Panel>
    )
  }
