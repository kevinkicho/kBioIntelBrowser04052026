...
export const PharosPanel = memo(function PharosPanel({ targets, panelId, lastFetched }: { targets: PharosTarget[], panelId?: string, lastFetched?: Date }) {
  if (targets.length === 0) {
    return (
      <Panel title="Pharos Target Development" panelId={panelId} lastFetched={lastFetched}>
        <p className="text-slate-500 text-sm">No Pharos target data found for this molecule.</p>
      </Panel>
    )
  }
