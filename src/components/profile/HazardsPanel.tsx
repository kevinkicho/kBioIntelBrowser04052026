...
export const HazardsPanel = memo(function HazardsPanel({ hazards, panelId, lastFetched }: { hazards: GhsHazardData | null, panelId?: string, lastFetched?: Date }) {
  if (!hazards) {
    return (
      <Panel title="Hazard Classification (GHS)" panelId={panelId} lastFetched={lastFetched}>
        <p className="text-slate-500 text-sm">No GHS hazard data available for this molecule.</p>
      </Panel>
    )
  }
