...
export const PeptideAtlasPanel = memo(function PeptideAtlasPanel({ peptides, panelId, lastFetched }: PeptideAtlasPanelProps) {
  ...
  const groupedByProtein = peptides.reduce((acc, peptide) => {
  ...
  return (
    <Panel title={`PeptideAtlas Peptides (${peptides.length} peptides, ${proteins.length} proteins)`} panelId={panelId} lastFetched={lastFetched}>
