...
export const FooDBPanel = memo(function FooDBPanel({ compounds, panelId, lastFetched }: FooDBPanelProps) {
  if (!compounds || compounds.length === 0) {
    return (
      <Panel title="FooDB - Food Compounds" panelId={panelId} lastFetched={lastFetched}>
        <p className="text-slate-500 text-sm">No food compound data found in FooDB.</p>
      </Panel>
    )
  }

  return (
    <Panel title="FooDB - Food Compounds" panelId={panelId} lastFetched={lastFetched} className="space-y-4">
