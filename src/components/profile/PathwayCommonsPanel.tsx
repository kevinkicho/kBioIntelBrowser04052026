...
export const PathwayCommonsPanel = memo(function PathwayCommonsPanel({ results, panelId, lastFetched }: { results: PathwayCommonsResult[], panelId?: string, lastFetched?: Date }) {
  if (results.length === 0) {
    return (
      <Panel title="Pathway Commons" panelId={panelId} lastFetched={lastFetched}>
        <p className="text-slate-500 text-sm">No Pathway Commons data found for this molecule.</p>
      </Panel>
    )
  }
