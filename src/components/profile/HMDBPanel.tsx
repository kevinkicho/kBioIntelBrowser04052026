...
export const HMDBPanel = memo(function HMDBPanel({ metabolites, panelId, lastFetched }: HMDBPanelProps) {
  const list = Array.isArray(metabolites) ? metabolites : []
  const isEmpty = list.length === 0
  const title = isEmpty ? 'HMDB' : 'HMDB Metabolites'

  const sortOptions = useMemo(
    () => [
      ...alphaSortOptions<HMDBMetabolite>((m) => m.name || ''),
      ...numberSortOptions<HMDBMetabolite>((m) => m.molecularWeight ?? 0, {
        high: 'Highest MW',
        low: 'Lowest MW',
      }),
    ],
    ...
    >
      {!isEmpty && (
        <FilterablePaginatedList
          ...
          getKey={(m, idx) => `${m.hmdbId || m.name}-${idx}`}
          renderItem={(metabolite) => (
