...
export const UniprotPanel = memo(function UniprotPanel({ entries, panelId, lastFetched }: { entries: UniprotEntry[], panelId?: string, lastFetched?: Date }) {
  if (entries.length === 0) {
    return (
      <Panel title="Gene & Protein (UniProt)" panelId={panelId} lastFetched={lastFetched}>
        <p className="text-slate-500 text-sm">No protein/gene data found for this molecule.</p>
      </Panel>
    )
  }
