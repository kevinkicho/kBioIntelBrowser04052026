...
export const ToxCastPanel = memo(function ToxCastPanel({ data, panelId, lastFetched }: ToxCastPanelProps) {
  if (!data || !data.assays || data.assays.length === 0) {
    return (
      <Panel title="EPA ToxCast / Tox21" panelId={panelId} lastFetched={lastFetched}>
        <p className="text-slate-500 text-sm">No ToxCast toxicity data found for this chemical.</p>
      </Panel>
    )
  }
  ...
  return (
    <Panel title="EPA ToxCast / Tox21" panelId={panelId} lastFetched={lastFetched} className="space-y-4">
