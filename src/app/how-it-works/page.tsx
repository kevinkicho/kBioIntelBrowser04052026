...
import { RESEARCH_PLAYBOOKS, RESEARCH_TOOLS } from '@/lib/methods/researchToolCatalog'
...
type TabId = 'overview' | 'algorithms' | 'prompts' | 'tools' | 'funnel'
...
export default function HowItWorksPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && window.location.hash) {
      ...
      if (id === 'tools' || id === 'playbooks' || RESEARCH_PLAYBOOKS.some((p) => p.id === id)) {
        setTab('tools')
        if (RESEARCH_PLAYBOOKS.some((p) => p.id === id)) setOpenPlaybook(id)
        window.setTimeout(() => {
          document.getElementById(id)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
        }, 100)
      } else if (id === 'discover_rank' || ALGORITHM_CATALOG.some((a) => a.id === id)) {
        setTab('algorithms')
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="page-canvas">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-1.5">
          ...
          <HelperTip
            content="Transparent view of algorithms (deterministic, free public APIs), AI prompts (claim-bound / evidence-gated), and research tools for humans + agents. Nothing here invents Discover ranks."
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              <p className="text-[12px] text-slate-300">
                {RESEARCH_TOOLS.length} surfaces · {RESEARCH_PLAYBOOKS.length} playbooks for humans and agents
              </p>
              <HelperTip
                content="UI entry points, CLI commands, allowlisted copilot tools, and step-by-step research playbooks (disease→shortlist, CID deep-dive, pack→RH, compare, agent ops)."
                label="About research tools"
                testId="how-tools-overview-help"
                maxWidth="22rem"
              />
              <button
                type="button"
                ...
                className="text-[11px] text-indigo-400 hover:underline"
                data-testid="how-overview-to-tools"
              >
                Open Tools
              </button>
            </div>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-2 text-[11px] text-slate-500">
              ...
              <HelperTip
                content="Source of truth: docs/design/discovery-workbench-*.md, src/lib/discovery/, src/lib/ai/, src/lib/methods/researchToolCatalog.ts."
        ...
        {tab === 'tools' && <HowToolsTab initialPlaybookId={openPlaybook} />}
