 ...
 * Never hard-500s on model/upstream failure — refuse with 200 or validation 400.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    let ctx
    try {
      ctx = buildPackAiContext(body.pack as PackAiRequest['pack'])
    } catch (err) {
      return NextResponse.json(
        {
          ...
          mode,
          refused: true,
          refuseReason: err instanceof Error ? err.message : 'Invalid pack context',
        },
        { status: 200 },
      )
    }
    ...
    try {
      ...
      return NextResponse.json({
        ...
        mode,
        refused: true,
        refuseReason: err instanceof Error ? err.message : 'Chat threw',
      })
    }
    ...
    // Inventory / live free-API probes must never hard-FAIL pack AI as 500
    return NextResponse.json(
      {
        ok: false,
        refused: true,
        refuseReason: err instanceof Error ? err.message : 'Pack AI unexpected error',
      },
      { status: 200 },
    )
  }
