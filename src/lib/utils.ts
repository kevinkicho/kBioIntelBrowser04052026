...
//   - Per-source overrides live in `src/lib/analytics/timeouts.ts` (server-only,
//     reads tuned overrides from data/timeout-overrides.json). This module is
//     imported by client components, so it MUST stay free of Node-only deps.
