...
/**
 * PDB harvest leaf. HTTP / HTML / timeout / network are not EMPTY.
 * 404, missing id, and zero-hit JSON remain [].
 */
...
export async function getPdbStructuresByName(name: string): Promise<PdbStructure[]> {
  ...
  const searchRes = await timedFetch(SEARCH_URL, {
    method: 'POST',
  ...
  throwIfHttpFailed(searchRes, 'PDB')
  ...
  const results = await Promise.all(
    pdbIds.map(async (id) => {
      try {
        ...
        entryFailure = e instanceof Error ? e : new Error(String(e))
        return null
      }
    }),
  )
  ...
  if (structures.length === 0 && entryFailure) throw entryFailure
