...
/**
 * PharmGKB harvest leaf. HTTP / HTML / timeout / network are not EMPTY.
...
export async function searchPharmGKBGene(geneSymbol: string): Promise<PharmGKBGene[]> {
  ...
  if (!q) return []
  const searchUrl = `${BASE_URL}/gene?symbol=${encodeURIComponent(q)}`
  const searchRes = await timedFetch(searchUrl, { ...fetchOptions, timeoutMs: 8000 })
  if (isAbsentStatus(searchRes.status)) return []
  throwIfHttpFailed(searchRes, 'PharmGKB')
  const searchData = await searchRes.json()
...
export async function getPharmGKBGuidelines(drugId: string): Promise<PharmGKBGuideline[]> {
  ...
  if (!id) return []
  const guidelinesUrl = `${BASE_URL}/guideline?drugId=${encodeURIComponent(id)}`
  const guidelinesRes = await timedFetch(guidelinesUrl, { ...fetchOptions, timeoutMs: 8000 })
  if (isAbsentStatus(guidelinesRes.status)) return []
  throwIfHttpFailed(guidelinesRes, 'PharmGKB')
