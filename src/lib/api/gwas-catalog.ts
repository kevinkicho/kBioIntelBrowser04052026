...
/**
 * GWAS Catalog harvest leaf. HTTP / HTML / timeout are not EMPTY.
...
async function searchTraits(query: string, maxResults = 10): Promise<Array<{ trait: string; efoUri: string }>> {
  ...
  throwIfHttpFailed(res, 'GWAS Catalog')
...
export async function getGwasAssociationsByName(name: string): Promise<GwasAssociation[]> {
  let traits: Array<{ trait: string; efoUri: string }>
  try {
    traits = await searchTraits(name, 3)
  } catch {
    // Search index failed — try REST studies. If that also fails, throw (not EMPTY).
