...
/**
 * OpenAlex works. HTTP / HTML / timeout are not EMPTY.
...
function parseWorks(data: { results?: Record<string, unknown>[] }): OpenAlexWork[] {
...
export async function getOpenAlexWorksByName(name: string): Promise<OpenAlexWork[]> {
  ...
  throwIfHttpFailed(res, 'OpenAlex')
  const data = await res.json()
  return parseWorks(data)
}